...
class Science_Model extends CI_Model{
	...
	function save_science_details($schoolcode,$category,$fairid,$item_details){
		...
		//	echo '$item_code[$i] == '.$item_code[$i].'----$this->input->post==='.$this->input->post('item_code'.$item_code[$i]).'<br>';
			//if($item_code[$i] == $this->input->post('item_code'.$item_code[$i]))
			//{
			...
			$admn_no					=	$this->input->post($adm_no);
			 ...
			 $admn_no=$admn_category.$adno;
				...
				if($cnt1 > 0)//exist
				{
					...
					{//echo '<br>1<br>';
						$this->fair_exist($data,$exist_fair,$partid);
					}
		...
		//	echo '<br><br><br><br>$data_par==='.print_r($data_par).'<br>eeeeeee=='.@$_POST['edit_item'].'<br>';
			if(($data_par['flg']==3 || $data_par['flg']==4)&& @$_POST['edit_item']<>1)break;
		}//for
		return $data_par;
   ...
   //print_r($data);
	...
	//----------------------
	//***********science fair school enterd list***********
