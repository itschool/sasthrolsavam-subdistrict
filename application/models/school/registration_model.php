...
if($data['class_start']==11 && $data['class_end']==12)
	...
	function update_special_order_participant_details($team)
	{
		...
		$sub_district_code=$this->session->userdata('SUB_DISTRICT');
				...
				$this->db->from('participant_item_details pid');
				$result5	=	$this->db->get(); 
				$part_details5=$result5->result_array();
		$captaincounter =1;
		for($i=0;$i<$limit;$i++)
		{
		//echo '<br>'.$i.'<br>';
		...
		$admn	=	$this->input->post('txtADNO'.$i);
			...
			else
			{
					...
					$this->db->where('pid.school_code',$data['school_code']);
					$this->db->where('pid.item_code',$data['item_code']);
					$this->db->where('pid.spo_id',$data['spo_id']);
					$this->db->from('participant_item_details pid');
	...
	//	print_r($_POST);
		...
		for($i=1;$i<=$limit;$i++)
		{
			...
			$part_details1=$result1->result_array();
			$cnt1	=	count($part_details1);
				//echo '<br><br><br>ddd'.$cnt1.'---'.$admn_no.'--'.$schoolcode;	
				//print_r($part_details1);
			if($cnt1 > 0)
			{
				$data['participant_id']	=	$part_details1[0]['participant_id'];
				//$admn_no='2058';
				...
				else
				{
					...
					$result41	=	$this->db->get();
					$part_details41=$result41->result_array();
					//echo '<br><br><br>team==='.$part_details41[0]['team_no'];
					if($part_details41[0]['team_no']==0)
						$data['team_no']		=	$part_details4[0]['team_no']+1;
					else
					{
						$this->db->select('count(participant_id) as cnt');
						$this->db->where('pid.school_code',$data['school_code']);
						$this->db->where('pid.item_code',$data['item_code']);
						$this->db->where('pid.spo_id',$data['spo_id']);
						$this->db->from('participant_item_details pid');
						$result51	=	$this->db->get();
						$part_details51=$result51->result_array();
						if($part_details51[0]['cnt']>=$limit)
						$data['team_no']		=	$part_details4[0]['team_no']+1;
						else $data['team_no'] = $part_details41[0]['team_no'];
					}			
	...
	//print_r($_POST);
		...
		else
		{
			...
			if(@$sspart_details3[0]['is_captain']=='Y')//if deleted participant is captaion , make the one among the remaining group as captain
			{
				...
				{
					$this->db->where('spo_id',$sspart_details3[0]['spo_id']);
	...
	function check_basic_dtls($school_code,$admn_no,$participant_name,$class,$gender,$item_code=0)
	{
		...
		else $this->db->where('exhibition != ',2);
		
		$this->db->from('participant_item_details');
		$result_query	=	$this->db->get(); 
		$result			=	$result_query->result_array();
		//echo '<br><br><br>ddddddd';
		//print_r($result	);
		$cnt = count($result);
		$exist_flag=0;
		if($cnt > 0)
		...
		{//echo $participant_name.'--'.$values['participant_name'].'<br>';
	...
	function workexp_exb_delete_parti($schoolcode,$admno){
		...
		//echo '<br><br><br>fffff';
			...
			$sspart_details3=$ssresult3->result_array();
			$sscnt3=count($sspart_details3);
			//echo @$sspart_details3[0]['is_captain'];
			if(@$sspart_details3[0]['is_captain']=='Y')//if deleted participant is captaion , make the one among the remaining group as captain
			{
				$captain['is_captain'] = 'Y';
				//in scienc edrams max participants=8
	...
	function update_participant_item_details_structure()
	{
			/*$query1 = 'IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_NAME = participant_item_details AND COLUMN_NAME = start_number)
BEGIN
   ALTER TABLE participant_item_details ADD start_number INT(11)
END';*/
		$query_start_number = "SELECT COUNT( * ) as cnt FROM INFORMATION_SCHEMA.`COLUMNS`
WHERE `TABLE_SCHEMA` LIKE 'malappuram'
AND `TABLE_NAME` LIKE 'participant_item_details'
AND `COLUMN_NAME` LIKE 'start_number' ";
		$participant_item_details_structure_start_number		= $this->db->query($query_start_number);
		$ary_parti_start_number 								= $participant_item_details_structure_start_number->result_array(); 
		if($ary_parti_start_number[0]['cnt']==0)
		{
			$query_start_number1 = "ALTER TABLE participant_item_details ADD start_number INT(11) NOT NULL AFTER code_confirmed";
		
			$participant_item_details_structure_start_number1		=$this->db->query($query_start_number1);
		}
		
		$query_codeGeneratedFlag = "SELECT COUNT( * ) as cnt FROM INFORMATION_SCHEMA.`COLUMNS`
WHERE `TABLE_SCHEMA` LIKE 'malappuram'
AND `TABLE_NAME` LIKE 'participant_item_details'
AND `COLUMN_NAME` LIKE 'codeGeneratedFlag' ";
		
		$participant_item_details_structure_codeGeneratedFlag		= $this->db->query($query_codeGeneratedFlag);
		$ary_parti_codeGeneratedFlag 								= $participant_item_details_structure_codeGeneratedFlag->result_array(); 
		if($ary_parti_codeGeneratedFlag[0]['cnt']==0)
		{
		$query_codeGeneratedFlag1 = "ALTER TABLE participant_item_details ADD codeGeneratedFlag INT(11) NOT NULL AFTER code_confirmed";
		
		$participant_item_details_structure_codeGeneratedFlag1		=$this->db->query($query_codeGeneratedFlag1);
		}
		
		$query_Exhibition = "DELETE FROM festival_master WHERE festival_master.fest_id = 13 LIMIT 1;";
		
		$participant_item_details_exhibition		= $this->db->query($query_Exhibition);
		//$ary_parti_exhibition 								= $participant_item_details_exhibition->result_array(); 
		
			$query1 = "UPDATE participant_item_details set participant_id= 243616 where pi_id= 1381  ";
		
			$participant_item_details1		=$this->db->query($query1);		
	
	}
	
	function get_result_item_details($item){
