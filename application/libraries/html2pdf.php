...
if (!defined('__CLASS_HTML2PDF__'))
{
	...
	class CI_HTML2PDF
	{
		var $langue			= 'en';		// langue des messages
		 ...
		 * @return	null
		 */
		function CI_HTML2PDF($sens = 'P', $format = 'A4', $langue='en', $marges = array(5, 5, 5, 8), $force_page = false)
		{
...
*/
		...
		*/	
		...
		{
			$content = preg_replace('/<page_header([^>]*)>/isU',	'<hr>'.CI_HTML2PDF::textGET('vue01').' : $1<hr><div$1>', $content);
			$content = preg_replace('/<page_footer([^>]*)>/isU',	'<hr>'.CI_HTML2PDF::textGET('vue02').' : $1<hr><div$1>', $content);
...
'.$content.'
		...
		*/		
		function loadAction($row)
		{
			...
			{
				CI_HTML2PDF::makeError(1, __FILE__, __LINE__, strtoupper($row['name']));
				return false;
			}
		}
...
//			$txt = utf8_decode(html_entity_decode($txt, ENT_QUOTES, 'UTF-8'));
			...
			while($x+$w>$right && $x<$right && strlen($txt))
			{
				...
				if (strlen($txt))
				{
					...
					$nb++;
					if ($nb>1000) CI_HTML2PDF::makeError(2, __FILE__, __LINE__, array($txt, $right-$left, $this->pdf->GetStringWidth($txt))); 
		...
		*/	
		function Image($src, $sub_li=false)
		{
			...
			if (count($infos)<2)
			{
				CI_HTML2PDF::makeError(6, __FILE__, __LINE__, $src);
				return false;
			}
...
//				$this->maxX = max($this->maxX, $x+$w);
		...
		*/	
		function Rectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
		{
			...
			if ($background['image'])
			{
				...
				if (count($i_infos)<2)
				{
					CI_HTML2PDF::makeError(6, __FILE__, __LINE__, $i_name);
					return false;
				}
		...
		*/	
		function o_H1($param)
		{
			...
			$this->style->save();
			$this->style->value['font-bold'] = true;
			$this->style->value['font-size'] = $this->style->ConvertToMM('60px');
...
//			if ($this->style->value['height']) $h = $this->style->value['height'];
		...
		*/	
		...
		function makeError($err, $file, $line, $other = null)
		{
			...
			{
				case 1:
					$msg = (CI_HTML2PDF::textGET('err01'));
					$msg = str_replace('[[OTHER]]', $other, $msg); 
					break;
					
				case 2:
					$msg = (CI_HTML2PDF::textGET('err02'));
				...
				case 3:
					$msg = (CI_HTML2PDF::textGET('err03'));
					$msg = str_replace('[[OTHER]]', $other, $msg); 
					break;
					
				case 4:
					$msg = (CI_HTML2PDF::textGET('err04'));
					$msg = str_replace('[[OTHER]]', print_r($other, true), $msg); 
					break;
					
				case 5:
					$msg = (CI_HTML2PDF::textGET('err05'));
					$msg = str_replace('[[OTHER]]', print_r($other, true), $msg); 
					break;
					
				case 6:
					$msg = (CI_HTML2PDF::textGET('err06'));
					$msg = str_replace('[[OTHER]]', $other, $msg); 
					break;	
					
				case 7:
					$msg = (CI_HTML2PDF::textGET('err07'));
					break;	
			}
			echo '<span style="color: #AA0000; font-weight: bold;">'.(CI_HTML2PDF::textGET('txt01')).$err.'</span><br>';
			echo (CI_HTML2PDF::textGET('txt02')).' '.$file.'<br>';
			echo (CI_HTML2PDF::textGET('txt03')).' '.$line.'<br>';
			echo '<br>';
			echo $msg;
			exit;	
		}
