...
<table width="100%" border="0" cellspacing="0" cellpadding="6" align="center" class="heading_tab" style="margin-top:15px;">
  <tr>
	...
	<? echo $fairname[0]['fairName']; ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <?php echo 'Festival  : '.$festname[0]['fest_name']; ?></th>
		...
		<tr>
			...
			<?php if($this->session->userdata('USER_GROUP') == 'A' || $this->session->userdata('USER_GROUP') == 'W' || $this->session->userdata('USER_GROUP') == 'U'){;?>
		 	<td id="result_confirm_single<?php echo $j;?>" align="center" class="table_row_first">
				<?php if($single[$j]['is_confirmed'] == 'Yes' && ($this->session->userdata('USER_GROUP') == 'A' || $this->session->userdata('USER_GROUP') == 'W')) { 
				$itemname	=	str_replace("'","",$single[$j]['item_name']);
				?>
					<a href="javascript:resetResultConfirmation('<?php echo $single[$j]['item_code'] ?>', 
						'<?php echo $single[$j]['item_code'].'&nbsp;-&nbsp;'.$single[$j]['item_name']; ?>',
						'result_confirm_single<?php echo $j;?>');" 
					onClick="javascript:resetResultConfirmation('<?php echo $single[$j]['item_code'] ?>', 
						'<?php echo $single[$j]['item_code'].'&nbsp;-&nbsp;'.$itemname; ?>',
 ...
 <?php
	...
	?>
		<tr>
			...
			<?php if($this->session->userdata('USER_GROUP') == 'A' || $this->session->userdata('USER_GROUP') == 'W' || $this->session->userdata('USER_GROUP') == 'U'){; //echo "<br />oo";?>
		 	<td id="result_confirm_group<?php echo $j;?>" align="center" class="table_row_first">
				<?php if($itempart[$j]['is_confirmed'] == 'Yes' && ($this->session->userdata('USER_GROUP') == 'A' || $this->session->userdata('USER_GROUP') == 'W')) { ?>
