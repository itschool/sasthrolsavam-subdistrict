...
<page_footer>
 ...
 <table width="100%" align="center" >
    <tr>
    	<td align="center" class="style1" height="27" width="800"><?php 
		if($_POST['txtFair']==1)echo 'Science Fair';
		else if($_POST['txtFair']==2)echo 'Mathematics Fair';
		else if($_POST['txtFair']==3)echo 'Socialscience Fair';
		else if($_POST['txtFair']==4)echo 'Workexperience Fair';
		else if($_POST['txtFair']==5)echo 'IT Fair';
		 ?></td>
    </tr>
    ...
    <td align="center" class="style1">Total  participants list <?php if($date!=""){ echo ' on '.datetophpmodel($date); } ?></td>
         ...
         <br /><table width="87%" border="1" align="center" cellpadding="0" cellspacing="0">
          ...
          <?php
			...
			for($k=0;$k<count($lp);$k++){
					...
					if(($lp[$k]['school_code']==$school[$j]['school_code'])&&($lp[$k]['gender']=='B')){
						$no_lp_boys=$lp[$k]['cntlp'] + @$lpexhib[$school[$j]['school_code']]['B'];
					}
					else if(($lp[$k]['school_code']==$school[$j]['school_code'])&&($lp[$k]['gender']=='G')){
					$no_lp_girls=$lp[$k]['cntlp'] + @$lpexhib[$school[$j]['school_code']]['G'];
					}
				...
				for($k=0;$k<count($up);$k++){
					...
					if(($up[$k]['school_code']==$school[$j]['school_code'])&&($up[$k]['gender']=='B')){
						$no_up_boys=$up[$k]['upid'] + @$upexhib[$school[$j]['school_code']]['B'];
					}
					else if(($up[$k]['school_code']==$school[$j]['school_code'])&&($up[$k]['gender']=='G')){
					$no_up_girls=$up[$k]['upid'] + @$upexhib[$school[$j]['school_code']]['G'];
					}
				...
				for($k=0;$k<count($hs);$k++){
					...
					if(($hs[$k]['school_code']==$school[$j]['school_code'])&&($hs[$k]['gender']=='B')){
						$no_hs_boys=$hs[$k]['hsid'] + @$hsexhib[$school[$j]['school_code']]['B'];
					}
					else if(($hs[$k]['school_code']==$school[$j]['school_code'])&&($hs[$k]['gender']=='G')){
					$no_hs_girls=$hs[$k]['hsid']+ @$hsexhib[$school[$j]['school_code']]['G'];
					}
				...
				for($k=0;$k<count($hss);$k++){
					...
					if(($hss[$k]['school_code']==$school[$j]['school_code'])&&($hss[$k]['gender']=='B')){
						$no_hss_boys=$hss[$k]['hssid']+@$hssexhib[$school[$j]['school_code']]['B'];
					}
					else if(($hss[$k]['school_code']==$school[$j]['school_code'])&&($hss[$k]['gender']=='G')){
					$no_hss_girls=$hss[$k]['hssid']+@$hssexhib[$school[$j]['school_code']]['G'];
					}
					
				
				}
				
				if($no_lp_boys==0 && @$lpexhib[$school[$j]['school_code']]['B']>0)$no_lp_boys=@$lpexhib[$school[$j]['school_code']]['B'];
				if($no_lp_girls==0 && @$lpexhib[$school[$j]['school_code']]['G']>0)$no_lp_girls=@$lpexhib[$school[$j]['school_code']]['G'];
				
				if($no_up_boys==0 && @$upexhib[$school[$j]['school_code']]['B']>0)$no_up_boys=@$upexhib[$school[$j]['school_code']]['B'];
				if($no_up_girls==0 && @$upexhib[$school[$j]['school_code']]['G']>0)$no_up_girls=@$upexhib[$school[$j]['school_code']]['G'];
				
				if($no_hs_boys==0 && @$hsexhib[$school[$j]['school_code']]['B']>0)$no_hs_boys=@$hsexhib[$school[$j]['school_code']]['B'];
				if($no_hs_girls==0 && @$hsexhib[$school[$j]['school_code']]['G']>0)$no_hs_girls=@$hsexhib[$school[$j]['school_code']]['G'];
				
				if($no_hss_boys==0 && @$hssexhib[$school[$j]['school_code']]['B']>0)$no_hss_boys=@$hssexhib[$school[$j]['school_code']]['B'];
				if($no_hss_girls==0 && @$hssexhib[$school[$j]['school_code']]['G']>0)$no_hss_girls=@$hssexhib[$school[$j]['school_code']]['G'];
			}
			else
			{
				for($k=0;$k<count($lp);$k++){
          ...
          <tr>
            ...
            <td class="ety" style="border-top:1px #666666; border-right:1px #666666; padding:0px;" align="left"><?php echo wordwrap($school[$j]['school_code'].' - '.$school[$j]['school_name'],30,'<br/>');?></td>
