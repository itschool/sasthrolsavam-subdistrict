...
<table width="62%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#666666">
    ...
    <?php 
	 $t=date('Y-m-d',strtotime($data['start_time']));
	$partData = $this->prereport_model->participate_school_details($fair,$fest,1,$t);
	$partCount = count($partData);
	$j=0;
	 for($i=1;$i<=$partCount+5;$i++)
    {
	?>
     <tr>
        <td class="style4" align="center"><?php echo $i;?></td>
        <td class="style4" align="center">&nbsp;<?php echo @$partData[$j]['prefixCode'].@$partData[$j]['codeNo'];?></td>
        <td class="style4"align="center">&nbsp;<?php echo @$partData[$j]['school_code'];?></td>
