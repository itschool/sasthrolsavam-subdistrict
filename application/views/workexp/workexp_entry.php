...
<?php
if($exb==0)
 echo form_open('school/registration/save_details', array('id' => 'workexp_onEntry','name' => 'workexp_onEntry'));
 else
     ...
     <tr>
       <th height="32" colspan="2" style="border-bottom:1px solid #AEAEAE;">&nbsp;
         ...
         <input name="fairid" type="hidden" id="fairid" value="<?php echo "4";?>" />
          <input name="exb" type="hidden" id="exb" value="<?php echo $exb;?>" />
     ...
     <tr >
       ...
       <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="heading_tab" >
         <tr style="border-bottom:1px solid #AEAEAE;">
           ...
           <th width="10%"><span class="style1">&nbsp;Standard</span></th>
        <?php if(@$festid==4) {?>  <th width="10%" ><span class="style1">&nbsp;TYPE<br />
(</span><span class="style4">HSS/VHSS</span><span class="style1">)</span></th>  <?php  } ?>
           <th width="10%" ><span class="style1">&nbsp;Admission Number</span></th>
           <th width="13%" ><span class="style1">&nbsp;Gender</span></th>
	...
	// echo @$part_item_details[$r]['item_code'];
	 ...
	 if(@$part_item_details[$r]['item_code']==$item_code[$j]){ 
       ...
       <?php } ?>
         ...
         <tr <?php if($j%2==0) {?> bgcolor="#E6F7F5"  <?php } else?> bgcolor="#FFFFFF" >
          ...
          <?php if($spn==0) {?> <td rowspan="<?php echo $k; ?>" style="text-align:center"><span class="style5"><?php echo $j+1;?></span></td>
          <?php }?>
		   ...
		   //onBlur="javascript:return socialsciencemathsValidate()"
		   //onkeypress="javascript:return toUppercase(event,this)"'.''.''
		   ?></td>
         ...
         <!--  <td >&nbsp;<?php //if(@$part_item_details[$r]['item_code']==$item_code[$j]){ if(@$parti_flag == '1') { echo @$part_item_details[$r]['remarks']; } else {?><textarea class="text-input textarea"  id="remarks" name="remarks<?php //echo @$item_code[$j].$k;?>" rows="1" cols="20" onkeypress="return toUppercase(event,this)"><?php //echo  @$part_item_details[$r]['remarks'];?></textarea> <?php //} }else { if(@$parti_flag == '1') { echo @$part_item_details[$r]['remarks']; } else {?><textarea class="text-input textarea" id="remarks" name="remarks<?php //echo @$item_code[$j].$k;?>" rows="1" cols="20" onkeypress="return toUppercase(event,this)"></textarea> <?php //} }?></td>-->
            ...
            <?php if(@$confirm=="N")  {if(@$parti_flag == '1') { 
			//if($spn==0) {?> 
            <td  align="center"  ><?php if(@$part_item_details[$r]['admn_no']) {?><a href="#" onClick="return deleteParticipant(<?php echo "'".$item_code[$j]."'";?>,<?php echo "'".@$part_item_details[$r]['admn_no']."'";?>,<?php echo 0;?>);"><img src="<?php echo base_url()?>images/icon_cross.png"width="20" height="20" title="Delete"></a><?php } ?></td><?php //} 
			 }}?>
            
          <?php if(@$confirm=="N")  {if(@$parti_flag == '1') { if($spn==0) {?> <td align="center"  rowspan="<?php echo $k; ?>"><?php if(@$part_item_details[$r]['admn_no']) {?><a href="javascript:void(0);"  onclick="javascript:return editParticipant(<?php echo "'".$item_code[$j]."'";?>);"><img src="<?php echo base_url()?>images/edit.gif" width="20" title="Edit" height="20"></a><?php }?></td><?php }}}?>
     ...
     <tr>
       ...
       <input name="items" id="items" type="hidden" value="<?php echo $itms; ?>" />
		   ...
		   if(@$confirm=="N" && @$sub_district_details[0]['worexpo_confirm_data_entry']=="N")  {echo (@$parti_flag == '1') ? form_button('Edit', 'Edit', 'class="btnalt" onClick="javascript:fncEditPartDeatils()"').'&nbsp;&nbsp;'.form_button('Print', 'Print', 'class="btnalt" onClick="javascript:goto_print()"') : form_submit('Save', 'Save', 'class="btnalt" onClick="javascript:return socialsciencemathsValidate()"  '); }
		    else {
