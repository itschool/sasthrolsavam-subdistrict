...
class Admin_Users extends CI_Controller {
	...
	function index()
	{
		...
		$this->Content['user_types']	=	$this->General_Model->prepare_select_box_data('user_types', 'user_type_id, type_name','','-- Select User Type --','user_type_id');
		//var_dump($this->Content['user_types']);
	...
	function add_admin()
	{
		...
		if($this->input->post('userType')==6)
		{	$data['user_type'] = 4; $data['user_group']			=	'C';}
		else
		{
	...
	function generate_code_gen_admins ()
	{
		$sub_dist_admin_details	= $this->Admin_Users_Model->get_all_sub_dist_admins ();
		if (is_array ($sub_dist_admin_details) && count($sub_dist_admin_details) > 0)
		{
			...
			foreach ($sub_dist_admin_details as $sub_dist_admin_details)
			{
				$data['user_name']	= strtolower(str_replace(' ', '_',trim($sub_dist_admin_details['sub_district_name']))).'_codeuser';
				if (!$this->Admin_Users_Model->check_username_exists('', $data['user_name']))
				{
					$data['generated_password']	= substr(fncUuid (), 0, 8);
					$data['password']			= get_encr_password($data['generated_password']);
