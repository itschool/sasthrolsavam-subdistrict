...
class Resultentry extends CI_Controller {
	...
	{			
		...
		$this->Content['fest']		=	$fest;
		$this->Content['fair']		=	$fair;
		$this->Content['selected_item_id']			=	array();
	...
	{		//echo '<br><br><br><br>sssssss'.$item_id;
		...
		$check_permis	=	$this->General_Model->get_data('item_master','fairId', array('item_code' => $item_id));
		$fair_type		=	$this->session->userdata('FAIR_TYPE');
		
		if((@$check_permis[0]['fairId'] == $fair_type || @$fair_type == 0) && count($check_permis) > 0) {	
		...
		$fairid			=	$selected_fest_fair[0]['fairId'];
		$festid			=	$selected_fest_fair[0]['fest_id'];
		$this->Content['items']						=	$this->General_Model->prepare_select_box_data_special('item_master','item_code,item_name','item_code,CONCAT_WS(\' - \',item_code,item_name ) as item_name',array('fest_id' => $festid ,'fairId' => $fairid),'Select Item','item_code');
		$this->Content['selected_fair_id']			=	$fairid;
		$this->Content['selected_fest_id']			=	$festid;
		$this->Content['selected_item_id']			=	$item_id;
		$this->Content['selected_fair_name']			=	$this->General_Model->get_data('science_master','fairName', array('fairId' => $fairid));
