...
class Registration extends CI_Controller{ 
	...
	{
	//var_dump($_POST);
	//echo  'subdist----------'.$sub_district.'<br>user---------'.$fairId;
	...
	//echo '<br><br><br>fffffff==='.$fairid.'-'.$exb.'<br>';
	//print_r($fairdet);
	 /* if($exb==2)
	 echo '<br><br><br>-----------cata'.$category;*/
	...
	//	if($this->Content['flag']==1)
		...
		//	var_dump($this->Content['parti_details']);
			...
			{ 
			 ...
			 if(@$fairdet['error']<>'')$this->Content['parti_flag']	=	2;
...
//	echo "<br /><br />kiloooo";
	...
	$maths_delete_itemcode						=	 $this->Registration_Model->delete_details($schoolcode,$category,$fairid,$item_code,$admn,$parti_id); }
			...
			$admno	=	$item_code;
			$exb_delete_parti					=	 $this->Registration_Model->workexp_exb_delete_parti($schoolcode,$admno);
	}
		...
		if($fg == 1)
		{
			for($m=0;$m<$item;$m++)
			{
				...
				while($max_participants[$m]!=0){
					 ...
					 $basic_flag =0;
					 if(((($item_array[$m]['item_code'] != 138) || ($item_array[$m]['item_code'] != 147) || ($item_array[$m]['item_code'] != 164) || ($item_array[$m]['item_code'] != 182)) ) )//if maths magazine
			{
	...
	//echo $fg;
		//echo '<br><br><br>jjjjjjjj'. $basic_flag.'hhhhhhhhh';
		...
		if((@$basic_flag > 0) && ($admission_number > 0 || $admission_number != ''))
		{
		...
		else
		{
			for($p=1;$p<6;$p++) { 
				...
				if(@$_POST['txtStandard'.$p] == 11 || @$_POST['txtStandard'.$p] == 12)
				{//echo '<br><br>jjjj';
				 ...
				 if($admn_no > 0)
				 {
				 $basic_flag = $this->Registration_Model->check_basic_dtls($schoolcode,$admn_no,@$_POST['name_participant'.$p],@$_POST['txtStandard'.$p],@$_POST['txtgender'.$p],2);
		...
		else
		{
			...
			if($fairid	==	1)
			{
				...
				if(@$science_details_save['flg']==3)	
				{
				 ...
				 else if(@$science_details_save['flg']==4)
				{
			...
			if($fairid	==	2)
			{
				@$maths_details_save						=	 $this->Maths_entry_Model->save_maths_details($schoolcode,$category,$fairid,$this->Content['item_details']);
					...
					$this->Content['error']		=	"Participants Details of ".@$maths_details_save['name']." Not Saved Admission no already exist";	
					 ...
					 else if(@$maths_details_save['flg']==4)
				{
						$this->Content['error']="Same Admission number with different names";
			...
			//	 echo "<br /><br><br><br /><br><br><br /><br><br>ggkeriii";
			// print_r($this->Content['item_details']);
			    if($exb==0){
				//echo "oo".$exb;
...
function school_entry_print($schoolcode,$fairid,$exb){
	$this->template->write_view('menu', 'menu', '');	
	 ...
	 if($exb==0){
	  ...
	  {
	  $this->Content['item_details_lp']							=	 $this->Registration_Model->get_item_details_exb(1,$fairid);
	 ...
	 $this->template->write_view('content','socialscience/socialscience_entry_print',$this->Content);	
	  ...
	  if($fairid==4){
	 	 $this->template->write_view('content','workexp/workexp_entry_print',$this->Content);	
	 }
	  if($fairid==5){
	...
	$content					= $this->load->view('workexp/workexp_entry_print_pdf',$this->Content, true);
	else if($fairid==5)
